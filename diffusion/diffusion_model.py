...
class DiffusionModel(pl.LightningModule):
    ...
    def down_sampling(self, x):
        ...
        for i in range(len(self.down_channels) - 1):
            ...
            #                                        self.channels[i + 1]))
            x = DBlock(in_channels=self.down_channels[i],
                       out_channels=self.down_channels[i + 1])(x)
            l_out = l_out + [x]
        return l_out
    ...
    def up_sampling(self, x, l_film_pitch, l_film_noisy):
        ...
        for i in range(len(self.up_channels) - 1):
            ...
            x = UBlock(in_channels=self.up_channels[i],
                       out_channels=self.up_channels[i + 1])(x,
                                                             l_film_pitch[i],
                                                             l_film_noisy[i])
