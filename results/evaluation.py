...
with torch.no_grad():
    with open("results.npy", "wb") as f:
        ...
        for i in range(number_of_examples):
            ...
            init_u_f0, init_u_loudness, init_e_f0, init_e_loudness, init_e_f0_mean, init_e_f0_stddev = next(
                test_data)
            u_f0, u_loudness, e_f0, e_loudness, e_f0_mean, e_f0_stddev = next(
                test_data)

            u_f0 = torch.cat(
                [torch.Tensor(init_u_f0.float()),
                 torch.Tensor(u_f0.float())], 1)
            u_loudness = torch.cat([
                torch.Tensor(init_u_loudness.float()),
                torch.Tensor(u_loudness.float())
            ], 1)
            ...
            for j in range(n_step):
                out_f0, out_loudness = get_model_next_step(
                    model, u_f0[:, j:1999 + j].to(device),
                    u_loudness[:, j:1999 + j].to(device), out_f0.to(device),
                    out_loudness.to(device))

            out_f0, out_loudness = out_f0.squeeze().detach(
            ), out_loudness.squeeze().detach()
            e_f0, e_loudness = e_f0[:, 1:].squeeze().detach(
            ), e_loudness[:, 1:].squeeze().detach()
            u_f0, u_loudness = u_f0.squeeze().detach(
            )[-1999:], u_loudness.squeeze().detach()[-1999:]
