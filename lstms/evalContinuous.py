...
dataset = ExpressiveDatasetPitchContinuous(list_transforms=list_transforms,
                                           eval=True)

model = ModelContinuousPitch.load_from_checkpoint(
...
for i in range(N_EXAMPLE):
    ...
    s_u_p = model_input[1:, :1]
    s_u_cents = torch.zeros_like(s_u_p)
    s_u_lo = model_input[1:, 1:122]

    s_e_pitch = model_input[1:, 122:123]
    s_e_cents = model_input[1:, 123:124]

    print(s_e_cents.shape)
    print(s_e_cents == np.zeros_like(s_e_cents))
    s_e_lo = model_input[1:, 124:245]
