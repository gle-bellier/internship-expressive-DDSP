...
with torch.no_grad():
    for i in range(number_of_examples):
        ...
        u_f0_norm = torch.squeeze(u_f0_norm, 0)
        rec_u_f0 = torch.tensor(sc_pitch.inverse_transform(u_f0_norm))
    
        u_loudness_norm = torch.squeeze(u_loudness_norm, 0)
        rec_u_loudness = torch.tensor(sc_loudness.inverse_transform(u_loudness_norm))


        plt.plot(u_f0.squeeze(), label="midi")
        plt.plot(rec_u_f0.squeeze(), label="model")
        plt.legend()
        plt.show()

        plt.plot(u_loudness.squeeze(), label="midi")
        plt.plot(rec_u_loudness.squeeze(), label="model")
        plt.legend()
        plt.show()
