...
if __name__ == "__main__":
    ...
    tb_logger = pl_loggers.TensorBoardLogger('logs/baseline2/{}/'.format(inst))
    trainer = pl.Trainer(
        gpus=1,
    ...
    train = BaseDataset(instrument=inst,
                        type_set="train",
                        data_augmentation=True,
                        list_transforms=list_transforms)
    test = BaseDataset(instrument=inst,
                       type_set="test",
                       data_augmentation=False,
                       list_transforms=list_transforms,
                       scalers=train.scalers)

    model = Baseline(600, 1024, 349, scalers=test.scalers)
